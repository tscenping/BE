...
export class AuthController {
	...
	@Post('/signin')
	@ApiOperation({ summary: '토큰을 발급받는다.', description: '처음 사이트에 들어와서 DB에 저장되는 유저면 명시해줘야 한다. 닉네임, 아바타 수정 화면을 프론트에서 띄워줄 수 있음. avatar랑 nickname이 null이라면 첫 가입이다.' })
	...
	@Patch('/signup')
	@ApiOperation({ summary: '회원가입', description: '이미 존재하는 유저인지 체크하고, 유저상태(status)를 online으로 업데이트 합니다' })
	@UseGuards(JwtAuthGuard)
	async signup( @GetUser() user: User, @Body() signupRequestDto: SignupRequestDto) {
	...
	@Post('/test/signin')
	@ApiOperation({ summary: '테스트용 계정 생성', description: '테스트용 계정 생성' })
	@ApiResponse({ status: 201, description: '토큰 발급 성공'})
	@ApiResponse({ status: 409, description: '이미 존재하는 닉네임'})
	...
	@Patch('/signout')
	@ApiOperation({ summary: '쿠키삭제', description: '200 OK 만 반환한다. 쿠키에 있는 access,refresh token를 지운다.' })
	@UseGuards(JwtAuthGuard)
